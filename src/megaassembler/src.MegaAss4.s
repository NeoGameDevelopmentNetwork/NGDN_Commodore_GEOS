...
;*** Bereichs und Programm-Adresse in Fehlermeldung schreiben.
...
:Err_AreaOverflow	tay
			...
			lda	#>DlgAreaOverflow
			pha
			jmp	FatalError_b		;Fehler beim assemblieren.

;*** Fehler: "Label überschritten".
:Err_LabelAdress	ldx	StackVector
			txs
...
::setVecLabels		lda	#<StartLabelArea
...
::setVecError		;lda	#<ErrCodeTab
			;ldx	#>ErrCodeTab
			lda	#<StartLabelArea
			ldx	#>StartLabelArea

::setVecData		sta	Vec_EndLabels1 +0
			stx	Vec_EndLabels1 +1

::fatalExit		lda	Opt_BootDrive
			jsr	NewSetDevice
			jsr	NewOpenDisk
			jmp	Mod_DoSysFile
...
;			lda	Hdr_IconDataEnd
			...
			LoadW	r0,ErrFileName
			jsr	DeleteFile		;Fehlerdatei löschen.

			lda	Opt_SymbTabDrive
			jsr	NewSetDevice		;Laufwerk aktivieren.
			jsr	NewOpenDisk		;Diskette öffnen.

			LoadW	r0,SymFileName
			jsr	DeleteFile		;Symboltabelle löschen.

			LoadW	r0,ExtFileName
			jsr	DeleteFile		;ext. Symboltabelle löschen.
