...
;*** Icons.
:RIcon_Name		w IconName
			b $00,$00,IconName_x,IconName_y
			b USE_COLOR_INPUT
...
:RIcon_ScrBar		w IconScrBar
			b $00,$00,IconScrBar_x,IconScrBar_y
			b USE_COLOR_INPUT
...
;*** ScrollBar auswerten.
:movePartList		lda	partCount		;Partitonen vorhanden?
			beq	:exit			; => Nein, Ende...

			lda	mouseYPos		;Mausposition einlesen.
			sec
			sbc	#< (LIST_BASE_Y *8)
			cmp	#1*8			;Eine Zeile hoch?
			bcc	movePrevPart		; => Ja, weiter...
			cmp	#4*8			;Eine Zeile runter?
			bcs	moveNextPart		; => Ja, weiter...

			ldx	partCount
			cpx	#5 +1			;Mehr als 5 Partitionen?
			bcc	:exit			; => Nein, Ende...

			cmp	#2*8			;Eine Seite zurück?
			bcc	movePrevPage		; => Ja, weiter...
			cmp	#3*8			;Eine Seite vorwärts?
			bcs	moveNextPage		; => Ja, weiter...

::exit			rts				;Mausklick ignorieren.

;*** Eine Seite nach oben.
:movePrevPage		lda	partCurPos		;Position einlesen.
			sec				;Eine Seite zurück.
			sbc	#5			;Anfang überschritten?
			bcs	:1			; => Nein, weiter...
			lda	#0			;Auf Anfang zurücksetzen.
::1			jmp	moveToPage		;Neue Seite anzeigen.

;*** Eine Seite nach unten.
:moveNextPage		lda	partCurPos		;Position einlesen.
			clc
			adc	#10
...
:moveToPage		cmp	partCurPos		;Position verändert?
			...
			jmp	printPartList		;Partitionsliste ausgeben.
::end			rts

;*** Eine Partition nach unten.
:moveNextPart		lda	partCurPos		;Position einlesen.
			clc
			adc	#5
...
:movePrevPart		lda	partCurPos		;Position einlesen.
			beq	:end
